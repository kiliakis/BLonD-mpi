...
# momProg = np.load("./newTrialMomentum.npy")
...
def track_emit(map_, turn):
    emRMS.append(np.pi*np.std(my_beam.dE)*np.std(my_beam.dt))
...
# tIt.add_function(track_emit, 1000)

#%%

intensity_ramp = np.linspace(0, targetIntensity, 10000)
...
for turn in range(n_iterations):
    ...
    if (approx == 0) or (approx == 2):
        totalInduced.induced_voltage_sum()
    elif (approx == 1) and (turn % n_turns_reduce == 0):
        totalInduced.induced_voltage_sum()
    ...
    if turn % 1000 == 0:
        with timing.timed_region('serial:track_emit'):
            my_beam.statistics()
            my_beam.gather_statistics()
            track_emit(map_, turn)
...
# for t in tIt:
#     if t < len(intensity_ramp):
#         my_beam.intensity = intensity_ramp[t]
#         my_beam.ratio = my_beam.intensity/my_beam.n_macroparticles
#     else:
#         break
