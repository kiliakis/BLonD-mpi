...
gconfig = {
    ...
    'err_formula': 'np.sqrt((1 - inputd["mean_dt"] / based["mean_dt"])**2 + (1 - inputd["std_dt"] / based["std_dt"])**2 + (1 - inputd["mean_dE"] / based["mean_dE"])**2 + (1 - inputd["std_dE"] / based["std_dE"])**2)',
    # 'ylim': [-7, -2],
    'ylim': [-4, 1],
    # 'xlim': [1.6, 36],
    ...
    'plots': {
        ...
        'lhc-base': {
            'in': {
                ...
                # 'seed3-': '{}/{}/exact/_p1000000_b1_s1000_t40000_w1_o4_N1_red1_mtw0_seed3_approx0_mpimpich3_lbreportonly,,,,_monitor100_tp0_precdouble_artdeloff_/10Aug20.13-01-24-25/monitor.h5',
            },
...
if __name__ == '__main__':
    ...
    for case in args.cases:
        ...
        for tech in args.techniques:
            ...
            bplot = plt.boxplot(np.log10(errordist), widths=width, positions=[
                                pos], **gconfig['boxplot'])
        ...
        if gconfig['boxplot']['vert'] == True:
            ...
            plt.xlim(gconfig['ylim'])
            plt.yticks(xtickspos, xticks, **gconfig['ticks'])
            yticks = ['{:1.0e}'.format(10**i) for i in range(gconfig['ylim'][0],
                                                                 gconfig['ylim'][1]+1)]
            plt.xticks(np.arange(gconfig['ylim'][0],
                                 gconfig['ylim'][1]+1), yticks, **gconfig['ticks'])
