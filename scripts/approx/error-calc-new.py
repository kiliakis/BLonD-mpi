...
parser = argparse.ArgumentParser(
    description='Calculate the error between each available input case.')


parser.add_argument('-i', '--indir', type=str, default=None,
                    help='Directory with the input raw .h5 files.')
...
parser.add_argument('-t', '--ts', type=int, default=[1], nargs='+',
...
quantities = ['profile', 'mean_dE', 'mean_dt', 'std_dt',
              'std_dE', 'bunch_position', 'bunch_length']
...
def calc_error(f1, f2, ts):
    ...
    errors = {}
    norm_dE = f1['default/dE_norm'].value
    norm_dt = f1['default/dt_norm'].value

    turns = f1['default/turns'].value
    turns = turns[:len(turns)-ts+1]
    real_slices = np.zeros(len(turns), dtype='int32')
    particles = np.zeros((2, len(turns)), dtype='int32')
    # particles[0] = f1['default/n_particles'].value[:-
    #                                                ts].reshape(len(turns)-ts+1)
    # particles[1] = f2['default/n_particles'].value[:-
    #                                                ts].reshape(len(turns)-ts+1)
    particles[0] = f1['default/n_particles'].value[:len(turns)][0]
    particles[1] = f2['default/n_particles'].value[:len(turns)][0]

    for key in quantities:

        f1data = f1['default'][key].value
        f2data = f2['default'][key].value
        if key in ['mean_dE', 'std_dE']:
            f1data /= norm_dE
            f2data /= norm_dE

        if key in ['mean_dt', 'std_dt', 'bunch_length', 'bunch_position']:
            f1data /= norm_dt
            f2data /= norm_dt

        # errors[key] = np.empty(len(f1data)-ts, dtype=float)
        # errors[key] = running_mean(f1data, ts)
        # f1sum = running_mean(f1data, ts)

        errors[key] = running_mean(np.sum((f1data - f2data)**2, axis=1), ts)
        if key == 'profile':
            real_slices = np.sum(
                (running_mean(f1data, ts) > 0) +
                (running_mean(f2data, ts) > 0),
                axis=1)
            errors[key] /= real_slices
        # else:
        #     errors[key] = running_mean(f1data-f2data, ts)**2
        ...
        #         errors[key][i] = np.sum(((f1sum - f2sum)/ts)**2) / real_slices[i]
        #     else:
...
if __name__ == '__main__':
    ...
    for ts in tss:
        ...
        for bunch, data in infiles_d.items():
            ...
            outh5.create_dataset('profile', (datasets, turns - ts+1),
                                 compression='gzip', compression_opts=4,
                                 dtype='float64')

            outh5.create_dataset('mean_dE', (datasets, turns - ts+1),
                                 compression='gzip', compression_opts=4,
                                 dtype='float64')

            outh5.create_dataset('mean_dt', (datasets, turns - ts+1),
                                 compression='gzip', compression_opts=4,
                                 dtype='float64')

            outh5.create_dataset('std_dE', (datasets, turns - ts+1),
                                 compression='gzip', compression_opts=4,
                                 dtype='float64')
            outh5.create_dataset('std_dt', (datasets, turns - ts+1),
                                 compression='gzip', compression_opts=4,
                                 dtype='float64')

            outh5.create_dataset('bunch_position', (datasets, turns - ts+1),
                                 compression='gzip', compression_opts=4,
                                 dtype='float64')

            outh5.create_dataset('bunch_length', (datasets, turns - ts+1),
                                 compression='gzip', compression_opts=4,
                                 dtype='float64')

            outh5.create_dataset('turns', (datasets, turns-ts+1),
                                 compression='gzip', compression_opts=4,
                                 dtype='int32')

            outh5.create_dataset('real_slices', (datasets, turns-ts+1),
                                 compression='gzip', compression_opts=4,
                                 dtype='int32')

            outh5.create_dataset('particles', (datasets, 2, turns-ts+1),
                                 compression='gzip', compression_opts=4,
                                 dtype='int32')
