...
gconfig = {
    ...
    # 'y_err_name': 'std',
    'xlabel': {'xlabel': 'Turn (x1000)', 'labelpad': 1, 'fontsize': 10},
    ...
    'fontsize': 10,
    'legend': {
        'loc': 'lower right', 'ncol': 2, 'handlelength': 1.5, 'fancybox': False,
        'framealpha': .7, 'fontsize': 10, 'labelspacing': 0, 'borderpad': 0.2,
...
if __name__ == '__main__':
    ...
    for idx, inputkey in enumerate(gconfig['inputkeys']):
        ax = axarr[idx]
        plt.sca(ax)
    # for plot in gconfig['plots']:
        ...
        for keyf in ind.keys():
            for key in (set(based.keys()) & set(ind[keyf].keys())):
                ...
                plt.plot(turns[::intv]/1000, error[::intv],
        ...
        plt.text(10, 0.1, gconfig['plots'][inputkey]['title'],
            fontsize=10, ha='left')
        # ax.set_title(gconfig['plots'][inputkey]['title'], 
        #             fontsize=10)
        #             # pad=-20)
                    # y=.8
            # **gconfig['title'])
    ...
    for file in gconfig['outfiles']:
        file = file.format(images_dir, this_filename[: -3], 'beseminar')
        print('[{}] {}: {}'.format(
