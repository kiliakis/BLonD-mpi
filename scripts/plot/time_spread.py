...
if __name__ == '__main__':
    ...
    for i, num_workers in enumerate(sorted(list(datadic.keys()))):
        ...
        for j, phase in enumerate(datadic[num_workers].keys()):
            ...
            # avgs = np.array([v['avg']
            #                  for v in datadic[num_workers][phase].values()])
            # # totals = np.array([np.sum([v['avg'] for v in datadic[num_workers][p].values()]) for p in datadic[num_workers].keys()])
            # avg = np.mean(avgs)
            # yerrlow = np.array([v['min']
            #                     for v in datadic[num_workers][phase].values()])
            # yerrlow = np.mean(avgs-yerrlow) / avg
            # yerrlow = np.mean(avgs-yerrlow)
            # yerrhigh = np.array([v['max']
            #                      for v in datadic[num_workers][phase].values()])
            # yerrhigh = np.mean(yerrhigh-avgs) / avg
            # yerrhigh = np.mean(yerrhigh-avgs)
            ...
            plt.bar(i+pos, avg, width=w, yerr=[[avg-yerrlow], [yerrhigh-avg]],
                    ...
                    capsize=5, alpha=0.7)
            pos += w
            # plt.errorbar(i, avg/avg, yerr=[[yerrlow], [yerrhigh]],
            #              markersize=8, color=figconf['colors'][phase],
            #              fmt='o', capsize=2)
