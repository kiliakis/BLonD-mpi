    ...
    'plot4': {
        'files': {
            res_dir+'raw/LHC-lb-mpich3-approx2/particles-report.csv': {
                    ...
                    'dic_rows': ['n'],
                    'dic_cols': ['turn_num', 'parts_avg',
                                 'parts_min', 'parts_max', 'times_avg',
                                 'times_min', 'times_max',
                                 'latencies_avg', 'latencies_min',
                                 'latencies_max'],
                    'subplots': {
                        'parts': {
                            'x_name': 'turn_num',
                        ...
                        'times': {
                            'x_name': 'turn_num',
                            'y_name': 'times_avg',
                            'y_min_name': 'times_min',
                            'y_max_name': 'times_max'
                        },
                        'latencies': {
                            'x_name': 'turn_num',
                            'y_name': 'latencies_avg',
                            'y_min_name': 'latencies_min',
                            'y_max_name': 'latencies_max'
                        }
                    }
            },
        ...
        'ylim': [0, 100],
        'figsize': (5, 3),
        'image_name': images_dir + 'lhc-lb-mpich-subplots.pdf'
...
if __name__ == '__main__':
    for plot_key, config in plots_config.items():
        plots_dir = {}
        for file, fconfig in config['files'].items():
            print(file)
            data = np.genfromtxt(file, delimiter='\t', dtype=str)
            header, data = list(data[0]), data[1:]
            temp = dictify(header, data, fconfig['dic_rows'], fconfig['dic_cols'])
