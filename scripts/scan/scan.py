...
if __name__ == '__main__':
    ...
    for tc in args.testcases:
        yc = yaml.load(
            open(this_directory + '/{}_configs.yml'.format(tc), 'r'))[args.environment]
        ...
        for rc in yc.run_configs:
            total_sims += yc.configs[rc]['repeats'] * len(yc.configs[rc]['w'])
        ...
        for analysis in yc.run_configs:
            ...
            for (p, b, s, t, r, w, o, time,
                ...
                N = int(max(math.ceil(w * o / common.cores_per_cpu), 1))
                ...
                for i in range(repeats):
                    ...
                    output = result_dir.format(
                        yc.case, analysis, job_name, timestr, 'output.txt')
                    error = result_dir.format(
                        yc.case, analysis, job_name, timestr, 'error.txt')
                    monitorfile = result_dir.format(
                        yc.case, analysis, job_name, timestr, 'monitor')
                    log_dir = result_dir.format(
                        yc.case, analysis, job_name, timestr, 'log')
                    report_dir = result_dir.format(
                        yc.case, analysis, job_name, timestr, 'report')
                    for d in [log_dir, report_dir]:
                        if not os.path.exists(d):
                            os.makedirs(d)
                            
                    # For the extract script
                    open(os.path.join(top_result_dir, yc.case, analysis, '.analysis'), 'a').close()
                    ...
                    if args.environment == 'local':
                        ...
                        batch_args = [
                            ...
                            common.tasks_per_node, str(math.ceil(w/N)),
                            common.cores, str(o),  # str(o),
                            common.time, str(time),
                            common.output, output,
                            common.error, error,
                            common.jobname, yc.case + '-' + analysis + job_name.split('/')[0] + '-' + str(i)]
