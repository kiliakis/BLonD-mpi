...
def matched_from_distribution_density_multibunch(beam, Ring, FullRingAndRF, distribution_options_list,
    ...
    *Function to generate a multi-bunch beam using the matched_from_distribution_density
    ...
    if intensity_list is None:
        ...
        n_macroparticles_per_bunch = beam.n_macroparticles / \
            n_bunches * np.ones(n_bunches)
    else:
    ...
    for RingAndRFSectionElement in FullRingAndRF.RingAndRFSection_list:
        for rf_system in range(RingAndRFSectionElement.n_rf):
            voltages = np.append(
                voltages, RingAndRFSectionElement.voltage[rf_system, 0])
            harmonics = np.append(
                harmonics, RingAndRFSectionElement.harmonic[rf_system, 0])
    ...
    for indexBunch in range(0, n_bunches):
        ...
        bunch = Beam(Ring, int(
            n_macroparticles_per_bunch[indexBunch]), intensity_per_bunch[indexBunch])
        ...
        beamIteration.n_macroparticles = int(
    ...
    *Function to generate a multi-bunch beam using the matched_from_distribution_density
    ...
    for indexBunch in range(0, n_bunches):
        ...
        if TotalInducedVoltage is not None:
            ...
            tau_induced_voltage_next_bunch = TotalInducedVoltageIteration.profile.bin_centers[(
                TotalInducedVoltageIteration.profile.bin_centers > left_edge)*(TotalInducedVoltageIteration.profile.bin_centers < right_edge)]
            induced_voltage_next_bunch = TotalInducedVoltageIteration.induced_voltage[(
                TotalInducedVoltageIteration.profile.bin_centers > left_edge)*(TotalInducedVoltageIteration.profile.bin_centers < right_edge)]
    ...
    if plot_option:
        plt.figure('Bunch train + induced voltage')
        plt.clf()
        plt.plot(TotalInducedVoltageIteration.profile.bin_centers, TotalInducedVoltageIteration.profile.n_macroparticles /
                 (1.*np.max(TotalInducedVoltageIteration.profile.n_macroparticles))*np.max(TotalInducedVoltageIteration.induced_voltage))
        plt.plot(TotalInducedVoltageIteration.profile.bin_centers,
                 TotalInducedVoltageIteration.induced_voltage)
        plt.show()
...
#------------------------------------------------------------------------
    ...
    temporary_beam = Beam(
        GeneralParameters, n_macro_per_bunch, intensity_per_bunch)
    ...
    if TotalInducedVoltage is not None:
        ...
        for it in range(n_iterations):
            ...
            induced_potential = - normalization_potential * \
                cumtrapz(
                    induced_voltage, dx=induced_voltage_coordinates[1] - induced_voltage_coordinates[0], initial=0)
            ...
            for indexBunch in range(n_bunches):
                ...
                length_dt = len(dt)
                length_dE = len(dE)
                beam.dt[indexBunch*length_dt:(indexBunch+1)*length_dt] = dt+(
                    indexBunch * bunch_spacing_buckets * bucket_size_tau)
...
#    normalization_potential = np.sign(eta_0)*charge/t_rev
    ...
    temporary_batch = Beam(GeneralParameters, int(
        n_macro_per_bunch*n_bunches), (intensity_per_bunch*n_bunches))
...
#                                          n_iterations_input = n_iterations)
    ...
    for index_batch in range(n_batch):
        beam.dt[index_batch*length_dt:(index_batch+1)*length_dt] = temporary_batch.dt + index_batch*(
            n_bunches-1)*bunch_spacing_buckets*bucket_size_tau + (index_batch)*batch_spacing_buckets*bucket_size_tau
        beam.dE[index_batch*length_dt:(index_batch+1)
                * length_dt] = temporary_batch.dE
...
#
            ...
            for indexBatch in range(n_batch):
                for indexBunch in range(n_bunches):
                    ...
                    length_dt = len(dt)
                    length_dE = len(dE)
                    beam.dt[(indexBunch+n_bunches*indexBatch)*length_dt:(indexBunch+n_bunches*indexBatch+1)*length_dt] = dt+(indexBunch *
                                                                                                                             bunch_spacing_buckets * bucket_size_tau) + indexBatch*(batch_spacing_buckets + (n_bunches-1)*bunch_spacing_buckets)*bucket_size_tau
                    beam.dE[(indexBunch+n_bunches*indexBatch) *
                            length_dE:(indexBunch+n_bunches*indexBatch+1)*length_dE] = dE
...
def compute_X_grid(normalization_DeltaE, time_array, potential_well,
    ...
    for i in range(len(J_array)):
        DELTA = np.sqrt((potential_well[i]-potential_well)
                        [potential_well <= potential_well[i]]/normalization_DeltaE)
